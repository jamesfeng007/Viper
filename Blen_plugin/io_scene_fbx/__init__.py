...
if "bpy" in locals():
    ...
    if "export_fbx_sdk" in locals():
        importlib.reload(export_fbx_sdk)        
...
class ExportSdkFBX(bpy.types.Operator, ExportHelper, IOFBXOrientationHelper):
    bl_idname = "export_scene_sdk.fbx"
    bl_label = "Export FBX SDK"
    bl_options = {'UNDO', 'PRESET'}
    ...
    def execute(self, context):
        ...
        from . import export_fbx_sdk
        return export_fbx_sdk.save(self, context, **keywords)
...
def menu_func_export_sdk(self, context):
    self.layout.operator(ExportSdkFBX.bl_idname, text="FBX_SDK (.fbx)")
